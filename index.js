...
// Root page (for Render URL visits)
app.get('/', (_req, res) => {
  res.type('html').send(`<!doctype html><html><head><meta charset="utf-8"><title>Nebula API</title></head><body style="font-family:system-ui;background:#0b0b0f;color:#e7e3ff;padding:24px"><h1>Nebula API</h1><p>Сервер работает.</p><ul><li><a href="/api/health" style="color:#8a5cff">/api/health</a></li></ul><p>Клиент нужно развернуть отдельно (GitHub Pages / Netlify) и установить API_BASE на этот домен.</p></body></html>`);
});
// Catch-all to avoid "Cannot GET /"
app.get('*', (_req, res) => {
  res.redirect('/');
});
